...
Make_Normal_File::Make_Normal_File()
{
    try {
        profile_output_file_path();

    } catch (...) {
        qDebug() << "Make_Normal_File::Make_Normal_File";
        throw;
    }
}
...
const string Make_Normal_File::make_normal_file(const QString &file_path)
{
    try {
        // 送入文件,进行处理
        tackle_file(file_path);
...
ifstream Make_Normal_File::input_file_open(const string &input_file_path)
{
    try {
        ifstream ifs;
        ...
        qDebug() << "Make_Normal_File::input_file_open";
        throw;
    }
}

ofstream Make_Normal_File::output_file_open(const string &out_file_path)
{
    try {
        ofstream ofs;
        ...
        qDebug() << "Make_Normal_File::output_file_open";
        throw;
    }
}

void Make_Normal_File::tackle_file(const QString &file_path)
{
    try {
        // 获取文件的输入流
        ...
        // 加载进容器中
        load_datas(body_datas);

    } catch (...) {
        qDebug() << "Make_Normal_File::tackle_single_file";
        throw;
    }
}

vector<vector<string>> Make_Normal_File::tackle_solo_file_get_all(const ifstream &ifs)
{
    try {
        string s;
...
//        test_datas(all_array);
        ...
        qDebug() << "Make_Normal_File::tackle_solo_file_get_all";
        throw;
    }
}

void Make_Normal_File::profile_target_data_params(vector<vector<string>> &all_array,
                                                 size_t &rows_num,
                                                 size_t &cols_num,
                                                 size_t &target_data_index)
{
    try {
        ...
        qDebug() << "Make_Normal_File::profile_target_data_params";
        throw;
    }
}

vector<vector<string>> Make_Normal_File::tackle_solo_file_get_target(vector<vector<string>> &all_array,
                                                                     size_t rows_num,
                                                                     size_t cols_num,
                                                                     size_t target_data_index)
{
    try {
        ...
        qDebug() << "Make_Normal_File::tackle_solo_file_get_target";
        throw;
    }
}

void Make_Normal_File::profile_div_index(const vector<vector<string>> &target_datas,
                                        size_t &end_head_begin_body)
{
    try {
        ...
        qDebug() << "Make_Normal_File::profile_div_index";
        throw;
    }
}

vector<vector<string>> Make_Normal_File::get_head_datas(const vector<vector<string>> &target_datas,
                                                       size_t end_head_begin_body)
{
    try {
        ...
        qDebug() << "Make_Normal_File::get_head_datas";
        throw;
    }

}

vector<vector<string>> Make_Normal_File::get_body_datas(const vector<vector<string>> &target_datas,
                                                       size_t end_head_begin_body)
{
    try {
        ...
        qDebug() << "Make_Normal_File::get_body_datas";
        throw;
    }
}


void Make_Normal_File::load_datas(const vector<vector<string>> &datas)
{
    try {
        for(size_t i = 0;i < datas.size();i++)
        {
            normal_datas.push_back(datas[i]);
        }

    } catch (...) {
        qDebug() << "Make_Normal_File::load_datas";
        throw;
    }
}

void Make_Normal_File::profile_output_file_path() noexcept
{
    try {
        normal_file_path = QCoreApplication::applicationDirPath() +
                "/" + normal_file_name;

    } catch (...) {
        qDebug() << "Make_Normal_File::profile_output_file_path";
        throw;
    }
}

void Make_Normal_File::save_datas()
{
    try {
        // 获取文件输出流
        ofstream ofs = output_file_open(normal_file_path.toStdString());

        // 将温度数据内容写出
        for(size_t i = 0;i < normal_datas.size(); i++)
        {
            for(size_t j = 0;j < normal_datas[i].size();j++)
            {
                ofs<<normal_datas[i][j]<<",";
            }
            ofs<<endl;
        }
        ...
        qDebug() << "Make_Normal_File::save_datas";
        throw;
    }
}

void Make_Normal_File::test_datas(const vector<vector<string>> &arrays)
{
    try {
        ...
        qDebug() << "Make_Normal_File::test_datas";
        throw;
    }
}
