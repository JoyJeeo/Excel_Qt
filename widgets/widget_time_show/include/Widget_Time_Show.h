#ifndef WIDGET_TIME_SHOW_H
#define WIDGET_TIME_SHOW_H

#include <QWidget>
#include "tools/include/Tools_Header_Proxy_01.h"

class Widget_Time_Show : public QWidget
{
    Q_OBJECT

public:
    Widget_Time_Show(QWidget *parent = nullptr);
    ~Widget_Time_Show();
...
private:
    ...
    Chart_Category_Value* initChart(const string& attri, // 指定为某个属性创建chart
                     ...
                     bool is_time_chart, // 选用哪一个target数据分析器件
                     int pic_choice = 0,
                     const QString& _xname = "Hr",
    ...
    Targetfile_Time_Valid_Data* time_datas; // 目标文件读取分析管理器
    Targetfile_Ration_Valid_Data* ration_datas; // 目标文件读取分析管理器
