    ...
    : QWidget(parent)
{
    try {
        ...
        time_datas = new Targetfile_Time_Valid_Data;
        ration_datas = new Targetfile_Ration_Valid_Data;
        // 初始化step分配器
        ...
        qDebug() << "Widget_Time_Show::Widget_Time_Show";
        throw;
    }
}

Widget_Time_Show::~Widget_Time_Show()
{


}

bool Widget_Time_Show::total_task(const QStringList &files_path)
{
    try {
        // 分析time文件数据
        const string time_target_file_path = src_file_manager->time_task(files_path);
        this->setWindowTitle(get_window_title().c_str());
        // 解析生成的target_file数据读入程序中
        time_datas->total_task(time_target_file_path);

        // 分析ration文件数据
        const string ration_target_file_path = src_file_manager->ration_task();
        // 解析生成的target_file数据读入程序中
        ration_datas->total_task(ration_target_file_path);
        ...
        qDebug() << "Widget_Time_Show::total_task";
        throw;
    }
}

const string Widget_Time_Show::get_window_title() noexcept
{
    // 由于不同文件夹中会有同名文件，因此使用绝对路径的方式加以文件区分
    return "Analysis file：" + src_file_manager->get_input_file_path();
}

void Widget_Time_Show::while_draw(int row_obj_nums)
{
    try {
        // 获取attri -> [site][part]的series数据,存于map<string,vector<vector<double>>>中
        map<string,map<string,map<string,double>>> time_series_datas =
                time_datas->get_series_datas();
        map<string,map<string,map<string,double>>> ration_series_datas =
                ration_datas->get_series_datas();

        // 获取target_file中有效的属性label
        auto labels = time_datas->get_labels();
        ...
        // 用来专门计算图片chart的个数；i需要负责labels的查看
        size_t counter = 0;
        // 设定选用哪一个target数据处理器
        bool is_time_chart = true;
        for(size_t i = 0;i < labels.size();i++,counter++) // labels.size()
        {
            ...
            QMap<string,QMap<string,QPointF>> time_site_points = get_matrix_pointF(time_series_datas[attri],
                                                                           time_datas->get_scatter_site(),
                                                                           time_datas->get_scatter_part(),
                                                                           is_time_chart
                                                                           );

            // 获取初始化好的chart
            Chart_Category_Value* time_chart = this->initChart(attri,time_site_points,
                                           time_datas->get_scatter_site(),
                                           time_datas->get_scatter_part(),
                                           is_time_chart
                                           );
            // (widget,row,col) 物件和在网格布局管理器中的横纵坐标位置
            this->pGridLayout->addWidget(time_chart,counter/row_obj_nums+1,counter%row_obj_nums+1);

            // 存储并保存
            // 填充pic 【会进行覆盖】
            Chart_Category_Value* pic_time_chart = this->initChart(attri,time_site_points,
                                             time_datas->get_scatter_site(),
                                             time_datas->get_scatter_part(),
                                             is_time_chart,
                                             2);
            bulid_pic_save(pic_time_chart,counter,pic_dir);
            ...
            // 存储并保存
            // 填充pic 【会进行覆盖】
            Chart_Category_Value* pic_ration_chart = this->initChart(attri,ration_site_points,
                                             ration_datas->get_scatter_site(),
                                             ration_datas->get_scatter_part(),
                                             !is_time_chart,
                                             2);
            bulid_pic_save(pic_ration_chart,counter,pic_dir);

        }
//        qDebug() << "i: " << i;
        // 最后一页是否被存储
        bulid_pic_save(nullptr,counter,pic_dir,true);


    } catch (...) {
        qDebug() << "Widget_Time_Show::while_draw";
        throw;
    }
}

Chart_Category_Value *Widget_Time_Show::initChart(const string &attri,
...
//        QString unit = QString::fromStdString(datas->get_attri_unit(attri));
        QString unit = "";
        if(is_time_chart) unit = QString::fromStdString(time_datas->get_attri_unit(attri));
        else unit = QString::fromStdString(ration_datas->get_attri_unit(attri));
        // 获取属性数值最值
//        pair<double,double> attri_value_XI = datas->get_attri_value_XI(attri);
        pair<double,double> attri_value_XI;
        if(is_time_chart) attri_value_XI = time_datas->get_attri_value_XI(attri);
        else attri_value_XI = ration_datas->get_attri_value_XI(attri);
        // 获取属性最值
//        auto attri_define_XI = datas->get_attri_define_XI(attri);
        pair<double,double> attri_define_XI;
        if(is_time_chart) attri_define_XI = time_datas->get_attri_define_XI(attri);
        else attri_define_XI = ration_datas->get_attri_define_XI(attri);
...
//        map<string,int> part_map = datas->get_part_map();
        ...
        if(is_time_chart) part_map = time_datas->get_part_map();
        else part_map = ration_datas->get_part_map();
        ...
        qDebug() << "Widget_Time_Show::initChart";
        throw;
    }
}


QMap<string,QMap<string,QPointF>> Widget_Time_Show::get_matrix_pointF(
...
//        test_for(ans);
        ...
        qDebug() << "Widget_Time_Show::get_matrix_pointF";
        throw;
    }
}

pair<double, double> Widget_Time_Show::profile_define_value_XI_proxy(pair<double,double> attri_define_XI,
                                                                     pair<double,double> attri_value_XI
                                                                     )
{
    /*
    ...
    */
    try {
        ...
        qDebug() << "Widget_Time_Show::profile_define_value_XI_proxy";
        throw;
    }
}

void Widget_Time_Show::profile_y_axis_ans(const pair<double, double> &attri_value_XI,
...
//            qDebug() << step;
        ...
        qDebug() << "Widget_Time_Show::profile_y_axis_ans";
        throw;
    }
}

int Widget_Time_Show::profile_y_axis_line_nums(const pair<double, double> &y_axis_XI,
                                                      double step)
{
    /*
        说明：
    ...
    */
    try {
        ...
        qDebug() << "Widget_Time_Show::profile_y_axis_line_nums";
        throw;
    }
}

const QString Widget_Time_Show::build_pic_dir()
{
    try {
        ...
        qDebug() << "Widget_Time_Show::build_pic_dir";
        throw;
    }
}

void Widget_Time_Show::clear_pic_dir(const QString &pic_dir)
{
    try {
        QDir dir(pic_dir);
        ...
        qDebug() << "Widget_Time_Show::clear_pic_dir";
        throw;
    }
}

void Widget_Time_Show::bulid_pic_save(Chart_Category_Value* pic_chart,
...
//                qDebug() << path;
    ...
    //        pic->show();
        ...
        qDebug() << "Widget_Time_Show::save_normal_pic";
        throw;
    }
}

void Widget_Time_Show::save_pic(QWidget *saved_widget, const QString &saved_path)
{
    try {
        // 抓取窗口生成图片并保存
        ...
        qDebug() << "Widget_Time_Show::save_pic";
        throw;
    }
}
